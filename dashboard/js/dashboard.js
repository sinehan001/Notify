(function ($) {
  ...
  $(function () {
    ...
    var table = $('#websiteData').DataTable({
      "ajax": {
        "url": 'dashboard/js/data1.txt',
        "dataSrc": "data"  // Specify the correct path to your array of data
      },
      "columns": [
        ...
        { data: 'url' },
        {
          data: 'verified', render: function (data) {
            return data ? '<i class="bi bi-check-lg text-success"></i> Verified' : '<i class="bi bi-x-circle text-danger"></i> Not Verified';
          }
        },
        { data: 'subscribers' },
        {
          ...
          render: function (data, type, row) {
            return '<button class="btn rounded-circle btn-primary p-2" data-toggle="tooltip" data-placement="right" title="Edit" onclick="editRow(' + row.id + ')"><i class="bi bi-pencil-square mr-0"></i></button>' +
              '<button class="btn btn-danger ml-2 p-2" data-toggle="tooltip" data-placement="right" title="Delete"  onclick="deleteRow(' + row.id + ')"><i class="bi bi-trash mr-0"></i></button>';
          }
        },
      ],
      dom: 'Bfrtip',
      buttons: [
          'copy', 
          {
              extend: 'csv',
              exportOptions: {
                  columns: [0, 1, 2, 3, 4] // Adjust the column indices accordingly
              }
          },
          {
              extend: 'excel',
          ...
          },
          {
              ...
              customize: function (win) {
                // Replace the <i> tag with "true" for the printed version of column 4 (index 3)
                $(win.document.body).find('#websiteData tbody td:nth-child(4)').each(function () {
  ...
  // Optional: Add DataTables styling for the buttons
  $('#websiteData').DataTable().buttons().container().appendTo('#websiteData_wrapper .col-md-6:eq(0)');



    const todayDateElement = document.getElementById('today-date'); if (todayDateElement) {
      // Check if the element has a value
      if (!todayDateElement.innerText.trim()) {
        // Get current date
        ...
        // Add the formatted date to the element with ID "today-date"
        todayDateElement.innerText = formattedDate;
      }
    }
